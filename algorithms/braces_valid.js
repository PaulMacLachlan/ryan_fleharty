...
function braces_valid(string){
    ...
    for(var i=0; i<string.length; i++){ //we finally read through the string
        ...
        if ( opens.indexOf(current) >= 0){ //check if that char is in the opens array
            ...
            var most_recent_open = current; //this will validate later close parens to make sure you're not mixing types
        }
            ...
            if(brace_set[key][1] < 0){
                return false;
            }; //whenever we close a parentheses, we want to test- Has the most recent open of a different type been closed?
            if (key != most_recent_open && brace_set[most_recent_open][1] > 0){; //check if you're mixing types improperly
                return false;
            };
